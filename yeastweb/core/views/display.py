...
from core.file.azure import CustomStorage
        ...
        try:
            ...
            if request.method == "POST":
                if "delete" in request.POST:
                    ...
                    delete_cell = CellStatistics.objects.get(
                        segmented_image=cell_image, cell_id=cell_id
                    )
                    delete_cell.delete()
            ...
            full_outlined = f"{uuid}/output/{image_file_name}.png"
            ...
            options = {
                "account_key": os.getenv("AZ_ACCESS_KEY"),
                "account_name": os.getenv("AZ_STORAGE_NAME"),
                "azure_container": "media",
            }

            storage = CustomStorage(**options)
            ...
            for i in range(1, cell_image.NumCells + 1):
                ...
                for channel_name in channel_order:
                    ...
                    if channel_name in ["mCherry", "GFP", "DAPI"]:
                        image_url = f"{uuid}/segmented/{image_name_stem}-{i}-{channel_name}_debug.png"
                    else:
                        image_url = f"{uuid}/segmented/{image_name_stem}-{channel_index}-{i}.png"
                    images[str(i)].append(storage.url(image_url))

                # Retrieve statistics for the cell
                try:
                    cell_table = CellTable(
                        CellStatistics.objects.all().filter(segmented_image=cell_image)
                    )
            ...
            all_files_data[str(uuid)] = {
                "MainImagePath": storage.url(full_outlined),
            ...
            return HttpResponse(
                f"Segmented image not found for UUID {uuid}", status=404
            )
